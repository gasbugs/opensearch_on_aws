...
module "eks" {
  ...
  cluster_addons = {
    ...
    amazon-cloudwatch-observability = {
      service_account_role_arn = module.irsa-cloudwatch-observability.iam_role_arn # CloudWatch Observability에 사용될 IRSA 역할
    }
  }
...
# CloudWatch Observability 권한을 불러옴
# CloudWatch Container Insights와 관련된 IAM 정책을 정의
...
# IRSA를 통해 CloudWatch 역할 생성 및 적용
module "irsa-cloudwatch-observability" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-assumable-role-with-oidc"
  version = "5.47.1"

  create_role  = true
  role_name    = "AmazonEKSCloudWatchObservabilityRole-${module.eks.cluster_name}"
  provider_url = module.eks.oidc_provider
  role_policy_arns = [
    data.aws_iam_policy.cloudwatch_observability_policy.arn
  ]
